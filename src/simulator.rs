...
pub fn run_simulation(curr_day: i64, latest_date: &NaiveDate, days_to_sim: &i64, num_sims: &i64, last_hist_price: &f64, setup_params: (&f64, &f64, &mut StdRng)) 
    -> Vec<(NaiveDate, f64, f64, f64, f64, f64)>
{
    let (stdev_p, drift, rng) = setup_params;
    let sim_date: NaiveDate = date_time::add_days_to_date(latest_date, &curr_day);
    let day_results: Vec<f64> = (0..*num_sims)
        .map(|_| oxide_stats::get_statistical_price(last_hist_price, stdev_p, drift, rng))
        .collect();
    ...
    if &curr_day < days_to_sim {
        let mut day_n_plus_one_result: Vec<(NaiveDate, f64, f64, f64, f64, f64)> =
            run_simulation(&curr_day + 1, latest_date, days_to_sim, num_sims, &mean_res, (stdev_p, drift, rng));
