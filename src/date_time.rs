...
pub fn get_naive_date_from_string(date_string: &str) -> NaiveDate {
    NaiveDate::parse_from_str(date_string, "%Y-%m-%d").unwrap()
}

pub fn days_between(end_date: &str, last_price_date: NaiveDate) -> i64 {
    let end_date_naive: NaiveDate = get_naive_date_from_string(end_date);
    (end_date_naive - last_price_date).num_days()
}
